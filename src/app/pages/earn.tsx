...
import { logoPath, siteName, tokenSymbol } from '@component/app/shared-data/shared-data'
  ...
  return (
    <div className={styles['earn--container']}>
      <div className={styles['earn--intro']}>
        <Image className='petgotchi-logo' src={logoPath} alt={`${siteName} logo`} width={120} height={120}/>
        <h1>How to Earn</h1>
        <p>
          {siteName} is built for grinders. Coins fuel your pet&apos;s care loop and stack toward
      ...
      <section className={styles['milestones--section']}>
        ...
        <p>Lifetime coins earned unlock tiers. Sprout, Bronze, and Silver each pay 500 {tokenSymbol}; Gold pays 1,500 and Diamond pays 3,000. Connect a Solana wallet in-game to claim.</p>
        <ul className={styles['tier--list']}>
          {milestones.map(( m ) => (
            <li key={m.threshold}>
              <strong>{m.reward}</strong> — {formatThreshold(m.threshold)} lifetime coins
              {m.tokenPayout != null && (
                <span> · claim {formatThreshold(m.tokenPayout)} {tokenSymbol}</span>
              )}
            </li>
          ))}
        </ul>
